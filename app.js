  ...
  var sql = `SELECT * FROM videofile`;
    con.query(sql, function (err, result) {
        ...
        if(result.length>0){
            ...
            //res.redirect("/");
            res.render("videoGallery",{session:req.session,data:result});
        }
        else
            res.redirect("/login");
    });
...
app.get('/singleVideo',function(req,res){
  var id=req.query.id;
  var sql = `SELECT * FROM videofile where id="${id}"`;
  console.log(sql);
    con.query(sql, function (err, result) {
        if (err) throw err;
        if(result.length>0){
...
app.get('/viewReviewRate',function(req,res){
  if(req.session.user=="Admin"){
  var sql = `SELECT id,(SELECT name FROM user where id=videocomment.uid) as name,(SELECT name FROM videofile where id=videocomment.vid) as video,review,rate FROM videocomment`;
    con.query(sql, function (err, result) {
        if (err) throw err;
        if(result.length>0){
...
app.get('/deleteVideo',function(req,res){
  ...
  var sql = `DELETE FROM videoFile WHERE id="${id}"`;
    con.query(sql, function (err, output) {
        if (err) throw err;
...
app.post('/singleVideo',function(req,res,next){
  ...
  var sql = `SELECT * FROM videofile where id="${vid}"`;
  console.log(sql);
  con.query(sql, function (err, result) {
    if (err) throw err;
    if(result.length>0){
...
app.post('/updateVideo', update_upload.single('file'), function(req, res) {
  ...
  fs.unlinkSync("assets"+path);
  // Insert the file name and path into mysql table
  var sql = 'UPDATE VideoFile set name=?, path=? WHERE id=?';
